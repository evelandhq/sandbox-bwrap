...
This package requires `eve` `>=0.25.0 <0.28.0` (eve's 0.x releases use
caret-incompatible minor bumps, so the verified three-minor window is written explicitly).
Compatibility is exercised against the pinned Eve 0.25.3, 0.26.2, and 0.27.12 releases. The backend implements the
